defmodule AndaWeb.UserAuthTest do
  use AndaWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Anda.Accounts
  alias Anda.Accounts.Scope
  alias AndaWeb.UserAuth

  import Anda.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, AndaWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      AndaWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        AndaWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
